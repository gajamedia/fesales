...
export class DetailprojekComponent implements OnInit {
  ...
  dataBahan: Bahan[] = [];
  dataKebTotalKain: { [id: number]: any[] } = {};
  ...
  materialLabels: Record<string, string> = {
    ...
    "009": "Kebutuhan Stik 35",
    "010": "Kebutuhan Fisher",
    "011": "Kebutuhan Galvanis 24",
    "012": "Kebutuhan Roda",
    "013": "Kebutuhan Bracket L",
    "014": "Kebutuhan Stik 55",
    "015": "Kebutuhan Stik 75",
    "016": "KebutuhanC Curved",
    "017": "Kebutuhan Bracket U",
    "018": "Kebutuhan AddOn",
    "019": "Kebutuhan Jasa Pasang",
    "020": "Kebutuhan Curved2",
    "021": "Kebutuhan Curved3",
    "022": "Kebutuhan Curved4",
  };

  totKebutuhanRel: { [itemCode: string]: { [id_project_detil: number]: KebutuhanValue } } = {};
    ...
    this.detailprojekService.getkain(id).subscribe({
      next: (res: any) => {
        ...
        if (res.details && res.details.length > 0) {
          ...
          this.kebtinggi[id] = Number(detail.tinggi_kain2 || 0);
        } else {
          this.kebtinggi[id] = 0;
        }
        //this.hitungTotal(id)
    ...
    this.detailbahanService.getbyIdDetailProjek(id.toString()).subscribe({
      next: (res: any) => {
        ...
        res.results.forEach((row: any) => {
          ...
          if (endpoint) {
            // otomatis → ambil qty dari endpoint
            this.loadKebutuhanRelnya(endpoint, this.expandedRows[id], itemCode);
          } else if (this.isManualQty(itemCode)) {
            // manual → tetap tampil di HTML dengan qty manual (default 0)
            if (!this.totKebutuhanRel[itemCode]) {
              this.totKebutuhanRel[itemCode] = {};
            }
            this.totKebutuhanRel[itemCode][id] = {
              total: row.qty ?? 0,   // qty manual kalau sudah ada
    ...
    filtered.forEach(row => {
      ...
      this.detailprojekService.kebutuhanRelnya(endpoint, params).subscribe({
        next: (res: any) => {
          ...
          this.totKebutuhanRel[itemCode][row.id_project_detil] = {
            total,
            itemName: row.item_name,
            //harga: res['harga_rel'] ?? res['harga'] ?? 0 // opsional
          };

          console.log("Update totKebutuhan", this.totKebutuhanRel);
        }
      });
    })
  }
  ...
  isManualQty(itemCode: string | undefined): boolean {
    return ['016','018','019'].includes(itemCode ?? '');
  }
  ...
  insertSelectedBahan(projectDetailId: number): void {
    ...
    selectedItems.forEach(item => {
      const code = item.item_code ?? '';   // pastikan string
      let qty = 0;

      // 1️⃣ Jika item butuh input manual (016,018,019), ambil dari item.qty
      if (['016','018','019'].includes(code)) {
        qty = item.qty ?? 0;
        console.log(`Manual qty untuk ${code} (${item.item_name}):`, qty);
      } 
      // 2️⃣ Jika bukan, ambil dari totKebutuhanRel
      else if (this.totKebutuhanRel[code] && 
         this.totKebutuhanRel[code][projectDetailId]) {
            console.log('Cek totKebutuhanRel:', {
              code,
              projectDetailId,
              data: this.totKebutuhanRel[code][projectDetailId]
            });

            qty = this.totKebutuhanRel[code][projectDetailId].total ?? 0;
            console.log(`Auto qty dari totKebutuhanRel untuk ${code} (${item.item_name}):`, qty);
          }
      ...
      console.log('Payload dikirim ke BE:', newDetail);

      this.detailbahanService.create(newDetail).subscribe({
        next: () => {
          insertCounter++;
          if (insertCounter === selectedItems.length) {
            console.log('Semua bahan berhasil di-insert dengan qty.');
            this.loadKebutuhanKain(projectDetailId);
            this.loadKebutuhanVitrase(projectDetailId);
            this.fetchDetailBahan(projectDetailId)
            // Reset modal & checklist
            this.showAddBahanModal = false;
            this.dataBahan.forEach(b => {
              b.selected = false;
              b.qty = undefined; // reset qty manual juga
            });
          }
        },
